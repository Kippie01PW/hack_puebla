...
export default function ProductHero() {
  ...
  return (
    <div className="relative w-full mx-auto rounded-3xl overflow-hidden shadow-xl" id="hero">
      ...
      <div className="relative h-96 md:h-[32rem] w-full">
        ...
        >
          <div className="ml-12 md:ml-20 max-w-md">
            <h1 className="text-2xl md:text-4xl font-bold leading-tight text-black dark:text-white">
              {slides[active].title}
            </h1>
            <p className="mt-3 md:mt-4 text-sm md:text-base/6 opacity-90 text-black dark:text-white">
              {slides[active].desc}
            </p>
            <a
              href={slides[active].href}
              className="
                mt-6 inline-block px-10 py-3 text-white font-semibold uppercase
      ...
      <button
        onClick={prevSlide}
        className="absolute top-1/2 left-4 md:left-8 -translate-y-1/2 w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-full bg-white/40 hover:bg-white/60 z-20 shadow"
      >
        <svg className="w-7 h-7 md:w-8 md:h-8 text-gray-800" fill="none" viewBox="0 0 24 24">
          <path stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7"/>
        </svg>
      </button>
      <button
        onClick={nextSlide}
        className="absolute top-1/2 right-4 md:right-8 -translate-y-1/2 w-12 h-12 md:w-14 md:h-14 flex items-center justify-center rounded-full bg-white/40 hover:bg-white/60 z-20 shadow"
      >
        <svg className="w-7 h-7 md:w-8 md:h-8 text-gray-800" fill="none" viewBox="0 0 24 24">
